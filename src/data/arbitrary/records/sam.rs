...
/// The allowable characters that can start an optional SAM tag.
const SAM_OPT_TAG_FIRST_CHARS: &[u8; 52] = b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";

/// The allowable characters that can be in the second position of an optional
/// SAM tag.
        ...
        match u.int_in_range(0_u8..=5)? {
            ...
            4 => {
                let hex = if self.bam_encodable {
                    // Ensure the string has an even length, and more
                    // efficiently use the arbitrary bytes when generating the
                    // string
                    let bytes = VecSpecs {
                    ...
                    // The string can be of any length, so use general-purpose
                    // StringSpecs
                    StringSpecs {
                        set: ByteSet::from(HEX_VAL),
                        max_len: self.max_len,
                        ..Default::default()
                    }
                    .make_arbitrary(u)?
